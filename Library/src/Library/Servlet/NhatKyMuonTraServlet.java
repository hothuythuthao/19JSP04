...
@WebServlet("/NhatKyMuonTraServlet")
public class NhatKyMuonTraServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public NhatKyMuonTraServlet() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (user == null) {
			...
			try {
				...
				List<UserBook> listUserBook = ubService.getAllUserBook();
				if (listUserBook.size() == 0) {
					...
					request.getRequestDispatcher("NhatKyMuonTra.jsp").forward(request, response);
				}
